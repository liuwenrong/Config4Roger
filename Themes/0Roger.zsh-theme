...
prompt_context() {
  ...
  #fi
    prompt_segment black default "%(!.%{%F{yellow}%}.)☁Roger♣liuwenrong"
}
...
prompt_time() {
 #"%Y-%m-%d %I:%M:%S
 #"%D{"%Y-%m-%d %I:%M:%S"}"
 #prompt_segment green black '%Y-%m-%d♆%I:%M:%S'  #显示路径 %s有问题
 prompt_segment white black '☂ %D{%Y-%m-%d %X}'  #2018-11-18 11:18:18
 #prompt_segment green black '%D{[%X]}'  #[11:18:18]
 #prompt_segment green black '♆%t'
 #prompt_segment green black '%W♆%t'
...
#PROMPT='╭─%{%f%b%k%}$(build_prompt) ➜ $(git_prompt_status)
PROMPT='╭─%{%f%b%k%}$(build_prompt1)
╰─%{$fg[$CARETCOLOR]%}➤ %{%f%b%k%}$(build_prompt)'
#╰─%{$fg[$CARETCOLOR]%}➤ %{%f%b%k%}$(build_prompt) %{$resetcolor%}'
